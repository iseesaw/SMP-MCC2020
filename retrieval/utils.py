...
def load_stopwords():
    ...
    """
    stopwords = set([])
    with open(CONFIG.get('STOP_WORDS'), "r", encoding="utf-8") as f:
...
def preprocess(session, chunk=5, delimiter='\t', attn_title=False):
    ...
    :return: dict
        {
            sampling: [(context, response)],
            chunking: [(context, response)]
        }
    """
    # 发帖人及其帖子
    ...
    if len(session.get('replys')) > chunk:
        ...
        if not len(ids):
            context = delimiter.join([r.get('content')
                                      for r in replys[:chunk]])
        ...
        # 发帖人回复位置不为空时，截取回复位置前 chunk 个回复构建 context
            ...
            for i in ids:
                start = i - chunk + 1
                context = delimiter.join([r.get('content')
                                          for r in replys[start:i]])
                # 是否在 context 首部拼接 title
                if attn_title and not start:
                    context = post.get('content') + delimiter + context
